...
import type { ClaudeStreamOptions, SSEEvent, TokenUsage, MCPServerConfig, PermissionRequestEvent, FileAttachment } from './claude-types';
import { isImageFile } from './claude-types';
...
function findClaudePath(): string | undefined {
  ...
  // Fallback: use which claude with enhanced PATH
  // This ensures we can find claude even if it's installed in non-standard locations
    ...
    async start(controller) {
      try {
        ...
        if (activeProvider && activeProvider.api_key) {
          ...
          // If neither legacy settings nor env vars provide a key, log an info message
          // Note: Claude CLI will read its own config from ~/.claude/settings.json,
          // so this is not an error if the user has configured it there.
          if (!appToken && !sdkEnv.ANTHROPIC_API_KEY && !sdkEnv.ANTHROPIC_AUTH_TOKEN) {
            console.log('[claude-client] No API key configured in CodePilot settings or environment variables.');
            console.log('[claude-client] Claude CLI will use its own configuration from ~/.claude/settings.json if available.');
          }
        }

        // Use acceptEdits mode as base; canUseTool handler below adds
        // auto-allow for read-only tools and Bash commands.
        ...
        const queryOptions: Options = {
          ...
          env: sdkEnv,
          // Load user settings from ~/.claude/settings.json to get API configuration
          settingSources: ['user'],
        };
        ...
        if (claudePath) {
          ...
          console.log('[streamClaude] Using Claude path:', claudePath);
        } else {
          console.warn('[streamClaude] Claude binary not found, relying on PATH');
        }

        // Ensure PATH includes common locations where Claude might be installed
        const home = os.homedir();
        const enhancedPath = [
        ...
        // Append enhanced PATH to existing PATH if not already present
        const existingPath = sdkEnv.PATH || '';
        const pathParts = existingPath.split(':');
        for (const extraPath of enhancedPath.split(':')) {
          if (!pathParts.includes(extraPath)) {
            pathParts.push(extraPath);
          }
        }
        sdkEnv.PATH = pathParts.join(':');

        console.log('[streamClaude] Enhanced PATH:', sdkEnv.PATH);
        ...
        // Permission handler: auto-allow read-only tools and Bash commands,
        // require user approval for file-editing tools.
        ...
        queryOptions.canUseTool = async (toolName, input, opts) => {
          ...
          // Auto-allow Bash/shell commands and MCP tool calls
          if (toolName === 'Bash' || toolName === 'bash' ||
              toolName.startsWith('mcp__')) {
            return { behavior: 'allow' as const };
          }

          // For file-editing tools (Write, Edit, MultiEdit, etc.),
          // send permission request to the client for user approval.
