...
pub fn install_local_skill<R: tauri::Runtime>(
    ...
    // Read description and category from SKILL.md
    let (description, category) = central_path
        .join("SKILL.md")
        .exists()
        .then(|| parse_skill_md(&central_path.join("SKILL.md")))
        .flatten()
        .map(|(_, desc, cat)| (desc, cat))
        .unwrap_or((None, None));
...
pub fn update_managed_skill_from_source<R: tauri::Runtime>(
    ...
    if let Err(err) = std::fs::rename(&staging_dir, &central_path) {
        ...
        // Only attempt fallback if staging_dir still exists
        if staging_dir.exists() {
            copy_dir_recursive(&staging_dir, &central_path)
                .with_context(|| format!("fallback copy {:?} -> {:?}", staging_dir, central_path))?;
            let _ = std::fs::remove_dir_all(&staging_dir);
        } else {
            // staging_dir was removed, try to restore from a backup or report error
            anyhow::bail!(
                "rename failed and staging_dir {:?} no longer exists: {}",
                staging_dir,
                err
            );
        }
    ...
    // Read description and category from SKILL.md
    // Always read from the final central_path to get the latest description
    let (description, category_from_md) = central_path
        .join("SKILL.md")
        .exists()
        .then(|| parse_skill_md(&central_path.join("SKILL.md")))
        .flatten()
    ...
    // Keep the existing category if it was set by user, otherwise use the one from SKILL.md
    let category = record.category.or(category_from_md);
...
pub fn list_git_skills<R: tauri::Runtime>(
    ...
    if let Some(subpath) = &parsed.subpath {
        ...
        if dir.is_dir() && dir.join("SKILL.md").exists() {
            let (name, desc, _cat) = parse_skill_md(&dir.join("SKILL.md")).unwrap_or((
                dir.file_name()
                    .unwrap_or_default()
                    .to_string_lossy()
                    .to_string(),
    ...
    if root_skill.exists() {
        let (name, desc, _cat) = parse_skill_md(&root_skill).unwrap_or(("root-skill".to_string(), None, None));
        out.push(GitSkillCandidate {
            name,
            description: desc,
            subpath: ".".to_string(),
    ...
    for base in [
        ...
        if let Ok(rd) = std::fs::read_dir(&base_dir) {
            for entry in rd.flatten() {
                ...
                let (name, desc, _cat) = parse_skill_md(&skill_md).unwrap_or((
                    p.file_name()
                        .unwrap_or_default()
                        .to_string_lossy()
                        .to_string(),
...
pub fn install_local_skill_from_selection<R: tauri::Runtime>(
    ...
    let (parsed_name, _desc, _cat) = parse_skill_md_with_reason(&skill_md)
...
pub fn parse_skill_md(path: &Path) -> Option<(String, Option<String>, Option<String>)> {
    parse_skill_md_with_reason(path).ok()
}

fn parse_skill_md_with_reason(path: &Path) -> Result<(String, Option<String>, Option<String>), &'static str> {
